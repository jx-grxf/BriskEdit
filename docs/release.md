...
- Two numbers, derived by `script/nightly_identity.sh`. `CFBundleVersion`/`sparkle:version` is the commit count on `dev`: monotonic, internal, and never shown in the app. The version users see is `<next release>-nightly.<nightlies since that release>`, so `0.6.2-nightly.3` is the third nightly on the way to 0.6.2. The next release is the version `dev` carries while it is unreleased, otherwise the next patch; a `-beta.N` suffix drops out because betas and nightlies lead to the same stable version. The sequence counts first-parent commits since the release, which is one per merged pull request.
...
3. **Build** on `macos-26`, in parallel with CI: package, sign (Developer ID), write notes with `script/nightly_release_notes.sh` (the pull requests merged since the previous nightly, or since the last release for the first nightly after one), build the Sparkle ZIP and appcast, notarize and staple, verify the artifacts, and upload them as a workflow artifact. Signing secrets exist only in this job.
4. **Publish** on Ubuntu runs only when CI, Resolve and Build all succeeded. It checks the artifacts against `SHA256SUMS`, attests them, and updates the single rolling `nightly` prerelease (never latest): archive and DMG first, then `appcast.xml` and `SHA256SUMS`, then notes. It moves the `nightly` tag to the commit (a failure only warns), keeps the two most recently uploaded archives, deletes older ones, and verifies the published feed and archive byte for byte.
...
Assets on the `nightly` release: `BriskEdit-Nightly.dmg` (stable download link), `BriskEdit-<version>.zip` (Sparkle), `appcast.xml`, `SHA256SUMS`.
